...
<div class="">
    <div class="flex">
        ...
        <div class="flex-1 pb-2 ml-4">
            <h3 class="text-blue mb-2 font-bold -mt-1" v-text="title"></h3>

            <p class="text-xs text-grey-darker mb-4">
                Posted by <a href="{{ route('profile', $thread->creator) }}" class="text-blue link">
                    {{ $thread->creator->isAdmin() ? 'admin' : $thread->creator->username }} {{ $thread->creator->isAdmin() ? '' : ($thread->creator->reputation . ' XP') }}
                </a>
            ...
            @if (count($thread->pictures))
                <thread-images :files="{{ $thread->pictures }}"></thread-images>
            @endif
        </div>
    </div>

    <div class="ml-4 border-b border-grey-lighter mt-2 pb-6">
